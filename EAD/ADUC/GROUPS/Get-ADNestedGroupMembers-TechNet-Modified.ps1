...
$modules = get-module | select -expand name
    ...
    { 
        ...
        {  
            ...
            { 
                $Props = @{Type=$nestedmember.objectclass;Name=$nestedmember.name;Title="";ParentGroup=$ADgroupname.name;Enabled="";Nesting=$nesting;DN=$nestedmember.distinguishedname;Comment=""} 
                 
                if ($nestedmember.objectclass -eq "user") 
                { 
                    $nestedADMember = get-aduser $nestedmember -properties enabled,title
                    $table = new-object psobject -property $props 
                    $table.enabled = $nestedadmember.enabled
                    $table.name = $nestedadmember.samaccountname
                    $table.Title = $nestedadmember.Title
                    if ($indent) 
                    { 
                    ...
                    { 
                    $table | select type,name,Title,parentgroup,nesting,enabled,dn,comment 
                    } 
                } 
...
<# COOL STUFF TO DO WITH THIS SCRIPT

$Groups = get-adgroup -Filter {sAMAccountName -like "*3060*Scientists"}
$groups | select Name
$ExportPath = 'C:\Temp'
...
$Groups = get-adgroup -Filter {sAMAccountName -like "ARSL*3060*"}
$groups = Get-ADGroup -Identity 'ARS-HQ-SmartCardExempt'
